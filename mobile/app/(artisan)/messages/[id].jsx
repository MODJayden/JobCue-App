...
const InChat = () => {
  ...
  useEffect(() => {
    if (id) {
      ...
      return () => {
        ...
        isInitialMount.current = true; // ✅ Reset on unmount
      };
    }
  }, [id, dispatch]);

  // ✅ FIXED: Cache participant with better fallback for customer profile picture
  useEffect(() => {
    ...
    if (participant && participant.userId) {
      ...
      // ✅ FIXED: Better profile picture fallback chain for customers
      const avatar = 
        participant.userId?.profilePicture || // First try userId.profilePicture
        participant.media?.profilePicture?.url || // Then try media.profilePicture.url
        participant.profilePicture || // Then try direct profilePicture
        cachedParticipantRef.current?.avatar; // Finally use cached value
  ...
  // ✅ FIXED: Better auto-scroll to last message
  useEffect(() => {
    if (messages?.length > 0 && flatListRef.current) {
      // Use different timing for initial load vs new messages
      const scrollDelay = isInitialMount.current ? 300 : 100;
      
      setTimeout(() => {
        flatListRef.current?.scrollToEnd({ animated: !isInitialMount.current });
  ...
  // ✅ ADDITIONAL: Scroll to end when content size changes (keyboard opens/closes)
